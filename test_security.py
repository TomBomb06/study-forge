...
def test_client_ip_ignores_the_client_supplied_forwarded_entry():
    ...
    per-IP limit, and — because the per-account login rule is checked before
    the password is verified — lock a known customer out of their own account.

    This test previously asserted that first entry was the right one, i.e. it
    encoded the vulnerability as the expected behaviour. Anyone "fixing" the
    resulting failure by changing the code would have reintroduced the hole.
    """
    class FakeReq:
...
def test_spoofed_forwarded_header_cannot_rotate_the_rate_limit_key():
    """The property that actually matters: no matter what the caller puts in
    the header, the key they are limited on does not move."""
    class FakeReq:
