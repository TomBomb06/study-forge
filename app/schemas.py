...
class QuizQuestion(BaseModel):
    ...
    def choices_nonempty(cls, v: list[str]) -> list[str]:
        if any(not c.strip() for c in v):
            raise ValueError("quiz choices must be non-empty")
        return v
...
class StudySetContent(BaseModel):
    ...
    summary: str = Field(min_length=50)  # the study-guide notes
    flashcards: list[Flashcard] = Field(min_length=5, max_length=100)
    quiz: list[QuizQuestion] = Field(min_length=3, max_length=50)
    test: list[TestQuestion] = Field(min_length=3, max_length=50)
    matching: list[MatchPair] = Field(min_length=4, max_length=30)
