...
def test_daily_xp_cap_blocks_level_farming(client):
    """Scores are declared by the client. Without a ceiling, a couple of dozen
    scripted 'perfect quiz' posts reached level 20 — which auto-applies a 20%
    coupon to every subscription payment."""
    r = client.post("/auth/signup", json={
        "email": f"farm-{uuid.uuid4().hex[:8]}@example.com", "password": "password123"})
    headers = {"Authorization": f"Bearer {r.json()['access_token']}"}
    last = None
    for _ in range(60):
        last = client.post("/gamify/event", headers=headers, json={
            "type": "quiz", "data": {"score": 200, "total": 200}, "tz_offset": 0})
        assert last.status_code == 200
    state = last.json()["state"]
    assert state["daily_xp"] <= gamify.DAILY_XP_CAP, state["daily_xp"]
    assert gamify.discount_for(state["level"]) < 20, (
        f"farmed to level {state['level']} — 20% coupon is reachable by script")
